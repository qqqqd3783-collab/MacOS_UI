...
KeyServices.platoboost = {
    ...
    New = function(serviceId, secret)
        ...
        return {
            Verify = function(key)
                local nonce = HttpService:GenerateGUID(false):sub(1,16)
                local body = {
            ...
            Copy = function()
                ...
                if success and response and response.StatusCode == 200 then
                    ...
                    if decoded.success and decoded.data.url then
                        return fSetClipboard(decoded.data.url)
                    end
                end
                return nil
            end
...
KeyServices.luarmor = {
    ...
    New = function(scriptId, discord)
        ...
        return {
            ...
            Copy = function()
                return fSetClipboard(tostring(discord))
            end
        }
    end
}
...
local function CreateKeySystem(config, callback)
    ...
    if keySettings.Service then
        ...
        if serviceModule then
            local args = keySettings.ServiceArgs or {}
    ...
    GetKeyBtn.MouseButton1Click:Connect(function()
        ...
        elseif keySettings.KeyLink then
            local success, result = pcall(function()
                local fSetClipboard = setclipboard or toclipboard
                return fSetClipboard(keySettings.KeyLink)
            end)
            if success then
                copySuccess = true
                copyResult = result
        ...
        if copySuccess then
            ...
            GetKeyBtn.BackgroundColor3 = Color3.fromRGB(255, 59, 48)
