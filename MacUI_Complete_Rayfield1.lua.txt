-- MacUI v2.0 with Rayfield-Style Configuration System
...
-- ==========================================
-- RAYFIELD-STYLE SETTINGS SYSTEM
-- ==========================================

local ConfigurationExtension = ".macui"

-- Settings Table (Rayfield-style structure)
local settingsTable = {
    General = {
        uiKeybind = {
            Type = 'bind', 
            Value = 'RightShift', 
            Name = 'MacUI Toggle Keybind',
            Element = nil
        },
        uiScale = {
            Type = 'number', 
...
local function setSetting(category, name, value)
    if settingsTable[category] and settingsTable[category][name] then
        settingsTable[category][name].Value = value
        if settingsTable[category][name].Element and settingsTable[category][name].Element.Set then
            settingsTable[category][name].Element:Set(value)
        end
    end
end
...
local function saveSettings()
    ...
    local filePath = ConfigFolder .. "/settings" .. ConfigurationExtension
    local success, result = pcall(function()
        local settingsData = {}
        for categoryName, settingCategory in pairs(settingsTable) do
            settingsData[categoryName] = {}
            for settingName, setting in pairs(settingCategory) do
                settingsData[categoryName][settingName] = {
                    Type = setting.Type,
                    Value = setting.Value,
                    Name = setting.Name
                }
        ...
        writefile(filePath, jsonString)
        print("[MacUI Settings] Saved to:", filePath)
    end)
    
    if not success then
        warn("[MacUI Settings] Failed to save:", result)
    end
end
        ...
        local success, result = pcall(function()
            ...
            for categoryName, settingCategory in pairs(decoded) do
                if settingsTable[categoryName] then
                    for settingName, setting in pairs(settingCategory) do
                        if settingsTable[categoryName][settingName] then
            ...
            print("[MacUI Settings] Loaded successfully")
            settingsInitialized = true
            return true
        end)
        
        if not success then
            warn("[MacUI Settings] Failed to load:", result)
            return false
        end
        return true
    end
    
    print("[MacUI Settings] No saved settings found, using defaults")
    settingsInitialized = true
    return false
end
...
function MacUI:Window(config)
    ...
    function self:Tab(name, icon)
        ...
        listLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            TabPage.CanvasSize = UDim2.new(0, 0, 0, listLayout.AbsoluteContentSize.Y + 125)
        end)
...
-- ==========================================
-- EXPORT SETTINGS API (RAYFIELD-STYLE)
-- ==========================================
