...
function MacUI:Window(config)
    ...
    self.SaveConfig = function()
        if config.ConfigName then
            SaveConfig(config.ConfigName, self.ConfigData)
        end
    end
    
    if config.ConfigName then
        local loaded = LoadConfig(config.ConfigName)
        if loaded then
            self.ConfigData = loaded
        end
    end
    ...
    function self:Tab(tabConfig)
        ...
        local TabLabel = create("TextLabel", {
            ...
            Position = UDim2.new(0, TabIcon and 36 or 10, 0, 0),
            BackgroundTransparency = 1,
            Text = TabName,
        ...
        TabButton.MouseButton1Click:Connect(function()
            for _, t in ipairs(self.Tabs) do
                ...
                tween(t.Label, 0.2, { TextColor3 = currentTheme.TextInactive })
                if t.Icon then
                    tween(t.Icon, 0.2, { ImageColor3 = currentTheme.TextInactive })
                end
            end
            
            TabPage.Visible = true
            tween(TabButton, 0.2, { BackgroundColor3 = currentTheme.TabActive })
            tween(TabLabel, 0.2, { TextColor3 = currentTheme.TextActive })
            if tab.Icon then
                tween(tab.Icon, 0.2, { ImageColor3 = currentTheme.TextActive })
            end
        ...
        function tab:ColorPicker(cfg)
            ...
            mainBtn.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    ...
                    holder.Size = UDim2.new(1, -20, 0, 208)
                    tween(pickerFrame, 0.2, { Size = UDim2.new(1, 0, 0, 166) })
                else
        ...
        function tab:Code(cfg)
            ...
            copyButton.MouseButton1Click:Connect(function()
                if setclipboard then
                    ...
                    copyButton.BackgroundColor3 = Color3.fromRGB(52, 199, 89)
...
function MacUI:Notify(cfg)
    ...
    tween(notification, 0.3, { Size = UDim2.new(0, 300, 0, 70) })
    
    local duration = cfg.Duration or 5
    
    closeBtn.MouseButton1Click:Connect(function()
        tween(notification, 0.2, { Size = UDim2.new(0, 0, 0, 0) })
        wait(0.2)
...
function MacUI:Dialog(cfg)
    ...
    create("UICorner", { Parent = DialogFrame, CornerRadius = UDim.new(0, 12) })
